...
function hasAnimal(zooAnimals, animalName) {
  //const lowerCaseAnimalName = animalName.toLowerCase();
  const animalIsIncluded = zooAnimals.includes(animalName);
  return animalIsIncluded;
}

/*function hasAnimal(zooAnimals, animalName) {
  const lowerCaseAnimalName = animalName.toLowerCase();
  const lowerCaseZooAnimals = zooAnimals.map(animal => animal.toLowerCase());
  return lowerCaseZooAnimals.includes(lowerCaseAnimalName);
}*/ 
// Diese Lösung ist aber nich besser mit find() zu lösen
